...
def api_bulk_status(batch_id):
    ...
    if is_complete and batch:
        # Clean up old batch results after they are complete
        bulk_results.pop(batch_id, None)
            ...
            if event_type == "message-delivered":
                if is_bulk_test:
                    ...
                    if "events" in target_dict[test_id_from_tag]:
                        start_time = target_dict[test_id_from_tag].get("events",{}).get("sent")
                        if start_time:
                ...
                if "events" in target_dict[test_id_from_tag]:
                    target_dict[test_id_from_tag]["events"]["sending"] = time.time()
    return "OK", 200
