    ...
    <script>
        ...
        function buildTable(data, tableId) {
            ...
            for (const row of data) {
                ...
                tr.innerHTML = `<td>${row.from_num}</td><td>${row.to_num}</td><td>${row.carrier_name}</td><td>${row.status}</td><td>${row.latency !== null ? row.latency.toFixed(2) : 'N/A'}</td>`;
    ...
    for dest_num, carrier_name in DESTINATION_NUMBERS:
        for from_data in from_numbers:
            for msg_type in message_types:
                ...
                bulk_results[batch_id][test_id] = {"from_num": from_data["number"], "to_num": dest_num, "carrier_name": carrier_name or 'N/A', "type": msg_type.upper(), "status": "Sending...", "latency": None}
...
def api_bulk_status(batch_id):
    ...
    return jsonify({"is_complete": is_complete, "sms_results": sms_results, "mms_results": mms_results})
